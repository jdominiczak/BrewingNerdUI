...
export default function RecipeIngredients(props) {
  let totalWeight = totalFermentables(props.recipe.fermentable_usages);
  ...
  return (
    <table className="table table-bordered">
      <tbody>
        ...
        {props.recipe.fermentable_usages.map( fermentable => { return renderRow("fermentable", fermentable, totalWeight)} )}
        {props.recipe.hop_usages.map( hop => { return renderRow("hop", hop)} )}
        {props.recipe.misc_usages.map( misc => { return renderRow("misc", misc)} )}
        {props.recipe.yeast_usages.map( yeast => { return renderRow("yeast", yeast)} )}
      </tbody>
    </table>
  )
}
